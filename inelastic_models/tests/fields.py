...
class SearchFieldTestCase(SearchBaseTestCase, test.TestCase):
    ...
    def setUp(self):
        ...
        tm = self.create_instance(
            name="Test1 one two three",
            email="test1@example.com",
            date=datetime.date(2015, 1, 1),
        )

        tm2 = self.create_instance(
            name="Test2 four five six", email="test2@example.com", date=None
        )
    ...
    def test_attribute_field(self):
        ...
        self.assertEqual(
            set(h.name for h in Model.search.execute().hits),
